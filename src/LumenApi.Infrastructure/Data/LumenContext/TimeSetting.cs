using System;
using System.Collections.Generic;

namespace LumenApi.Web;

public partial class TimeSetting
{
    public int Id { get; set; }

    public string? Time1 { get; set; }

    public string? Time11 { get; set; }

    public string? Time2 { get; set; }

    public string? Time22 { get; set; }

    public string? Time3 { get; set; }

    public string? Time33 { get; set; }

    public string? Time4 { get; set; }

    public string? Time44 { get; set; }

    public string? Time5 { get; set; }

    public string? Time55 { get; set; }

    public string? Time6 { get; set; }

    public string? Time66 { get; set; }

    public string? Time7 { get; set; }

    public string? Time77 { get; set; }

    public string? Time8 { get; set; }

    public string? Time88 { get; set; }

    public string? Time9 { get; set; }

    public string? Time99 { get; set; }

    public string? Time10 { get; set; }

    public string? Time101 { get; set; }

    public string? Time102 { get; set; }
}
